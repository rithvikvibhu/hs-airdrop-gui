...
async function readKey({ file, keyID, keyType, passphrase, isFile = true }) {
  console.log('readKey:', file, keyID, keyType, passphrase, isFile)
  ...
  switch (ext) {
    ...
    default: {
      const str = data.toString('utf8')
      console.log('str:', str, passphrase)
...
async function redeemStart({ keyType, privKey, keyId, hnsAddr, passphrase }) {
  console.log('Redeeming', keyType, privKey.slice(0, 20), keyId, hnsAddr)
...
/* Send progress updates (percentage, message-for-log) */
function updateRedeemStatus(percent, status) {
  console.log('Sending to renderer:', percent, status)
  BrowserWindow.getAllWindows()[0].webContents.send('redeem-update-status', {
    status,
    percent,
  })
}

/* This is called the 'redeem' event is emitted */
ipcMain.on('redeem', (event, arg) => {
  console.log(arg)
  redeemStart(arg)
})

// Don't really have to export anything, just for testing
export { redeemStart }
